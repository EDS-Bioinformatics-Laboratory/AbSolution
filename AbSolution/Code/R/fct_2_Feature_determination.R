...
Feature_1 <- function(path_base, grouping_by) {
    ...
    return(
        length(
            split(
                as.data.table(IMGT_parsed_index),
                by = grouping_by
            )
        )
    )
...
Feature__dataset <- function(
    ...
    seqs_with_strange_chars <- which(
        unname(
            sapply(
                DF_to_parse$NT_Whole, function(x) if (any(
    ...
    for (NT_region in NT_regions) {
        if (FWR1partial && NT_region == NT_regions[1]) {
            ignore_FWR1 <- c(
                ignore_FWR1, which(
                  nchar(unlist(DF_to_parse[, ..NT_region])) <=
                    nt_cutoff_length
              )
            )
    ...
    if (length(to_remove) >
        ...
        if (length(to_remove) >
            0) {
            fwrite(
                DF_to_parse[to_remove, ], file = paste0(output_path, "/", name_DF_to_parse, "_DISCARDED_bc_lenght-chars.txt"),
                sep = "\t"
            )
        }
    ...
    # cl <- parallel::makeForkCluster(max(1,detectCores() - 2), outfile =
    # paste(output_path, '/', name_DF_to_parse, '_ORF.outfile.txt', sep=''))
    # doParallel::registerDoParallel(cl) AA_output=
    # foreach(j=c(1:nrow(DF_to_parse)), .export =c('NT_regions'),
    # .packages=c('Biostrings','BiocGenerics','data.table','stringr'),.verbose=F,
    # .combine=rbind) %dopar% { unlist(subseq_func(as.character(DF_to_parse[j,
    # 'NT_Whole']),
    # width(as.character(DF_to_parse[j,..NT_regions])),FWR1partial,FWR4partial))
    # } parallel::stopCluster(cl)
    # colnames(AA_output)=c(paste0('AA_',c(regions,'Whole')), 'ORF')



    # cl <- parallel::makeForkCluster(max(1,detectCores() - 3), outfile =
    # paste(output_path, '/', name_DF_to_parse, '_ORF.outfile.txt', sep=''))
    # doParallel::registerDoParallel(cl) end=0 step_size=5000
    # tmp_AA_output=list() while ((end + 1) < nrow(DF_to_parse)) { start=end +
    # 1 print(paste(start, paste0(hour(Sys.time()), ':', minute(Sys.time()))))
    # end = end + step_size subsetDF=DF_to_parse[start:min(end,
    # nrow(DF_to_parse)),] tmp = foreach(j=c(1:nrow(subsetDF)), .export
    # =c('NT_regions'),
    # .packages=c('Biostrings','BiocGenerics','data.table','stringr'),.verbose=F,
    # .combine=rbind) %dopar% { unlist(subseq_func(as.character(subsetDF[j,
    # 'NT_Whole']), width(sapply(as.character(subsetDF[j,..NT_regions]),
    # function(z) if(z=='NA'){''}else{z})),FWR1partial,FWR4partial )) }
    # tmp_AA_output[[length(tmp_AA_output)+1]] = as.data.frame(tmp) }
    # parallel::stopCluster(cl) print('RBINDING')
    # AA_output=rbindlist(tmp_AA_output) print('RBOUND')
    # colnames(AA_output)=c(paste0('AA_',c(regions,'Whole')), 'ORF')

    # end.time <- Sys.time() time.taken_Parallel <- end.time - start.time
    ...
    AA_output <- foreach(
        subsetDF = i_DF_to_parse, .export = c(
            "NT_regions", "nt_changes", "subseq_func", "subseq", "GENETIC_CODE",
            "matchPattern", "width", "str_length", "translate_fun"
        ),
                  ...
                  as.data.frame(
                    (unlist(
                      subseq_func(
                        ...
                        width(
                          sapply(
                            as.character(subsetDF[, ..NT_regions]),
                            function(z) if (z ==
                              "NA") {
                              ""
                            } else {
    ...
    setDT(DF_to_parse)[,
        ORFs_match := c("Not Match", "Match")[(uniqueN(ORF) ==
            1) + 1], ID]
    ...
    while ((end + 1) < nrow(tmp_FBM)) {
        ...
        results <- foreach(
            d_rep = i_rep, d_recgerm = i_recgerm, .export = c(
                "bulk", "gravy", "aliphatic", "polar", "aIndex", "boman", "hmoment",
                "hydrophobicity", "mw", "pI", "crucianiProperties", "fasgaiVectors",
                "blosumIndices", "kideraFactors", "mswhimScores", "stScales", "tScales",
                "vhseScales", "zScales", "nt_changes", "lv_changes", "length_of_region",
                "count_elements", "locate_motif", "aa_changes", "group_freqs", "alakazam_properties",
                "Peptides_properties", "compare_sequences", "instaIndex_improved",
                "full_analysis", "NT_regions", "AA_regions", "hot_cold_mtfs", "mut_points_mtfs",
                "NGly_motif", "NGly_motif_pos", "FWR1partial", "FWR4partial", "stringdist"
            ),
            .packages = c("Biostrings", "BiocGenerics", "data.table", "stringr"),
            .verbose = F
        ) %dopar%
            {
                full_analysis(
                  as.data.table(rbind(d_rep, d_recgerm)),
        ...
        for (NT_region in NT_regions) {
            if ((NT_region == "NT_FWR1" && FWR1partial) || (NT_region == "NT_FWR4" &&
                ...
                if (index == repertoire_index) {
                  values_not_to_diff <- c(
                    values_not_to_diff, rep(0, length(nt_changes(sequence, seq_rec.germline, NT_region)))
                )
                }
