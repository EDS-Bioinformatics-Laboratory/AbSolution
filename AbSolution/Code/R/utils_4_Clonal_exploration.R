...
calculate_clone <- function(
    ...
    if (!(paste("Clone", AA_or_NT, region, clonotype, percentage, "simil", sep = "_") %in%
        colnames(seq_df))) {
        ...
        if (clonotype == "Reconstructed_germline") {
            ...
            tmp <- paste(
                ...
                seq_df[tmp_selected_rows, get(paste(AA_or_NT, "Whole", sep = "_"))],
                sep = "_______"
            )
...
draw_violinplots <- function(
    ...
    percentage = 100, freq_filter = 0, Selected_clones = NULL, dominance_threshold
) {
            ...
            if (nrow(subsubset_seq_df) >
                ...
                fig <- add_trace(
                  ...
                  hoveron = "points", text = paste(
                    tmp_frequencies$Clone_ID[which(!(tmp_frequencies$Clone_ID %in% Selected_clones$key))],
                    " Number of subclones=", tmp_frequencies$Number_of_unique_subclones[which(!(tmp_frequencies$Clone_ID %in% Selected_clones$key))],
    ...
    fig <- layout(
        fig, title = paste(
            c(
                "Relative frequency distribution of clones<br><i>using the criteria ",
                clonotype, "</i>"
            ),
            collapse = ""
        ),
...
draw_sharedclonesplot <- function(
        ...
        for (group_B in unlist(sets)) {
            if (group_A != group_B) {
                ...
                if (!(tmp_comparison %in% set_vector)) {
                  ...
                  tmp_fig <- plot_ly(
                    data = tmp_summary_df, x = ~Freq_X, y = ~Freq_Y, text = ~info,
                    hoverinfo = "text", key = tmp_summary_df$Clone_ID, color = ~shared,
                    colors = c("#e63946", "#2b2d42"),
                    marker = list(size = 10, opacity = 0.75)
                )
                  tmp_fig <- layout(
                    tmp_fig, xaxis = list(
                      title = paste("Log10 clonal frequency", group1, sep = " - "),
                      type = "log", exponentformat = "power", range = c(-5, 2)
                  ),
                    yaxis = list(
    ...
    sharedclonesplot <- subplot(
        list_figs, nrows = ceiling(length(list_figs)/4),
