...
filter_merged <- function(
    ...
    rows <- rows[which(
        rows %in% intersect(
            which(
                FBM[, which(merged_header == "AA_Whole_Replacement_muts")] >=
                  R_mut_threshold_min
            ),
            which(
                FBM[, which(merged_header == "AA_Whole_Replacement_muts")] <=
                    ...
                    if (VDJ_maximize_clones) {
                      ...
                      if (VDJ_normalized_per_sample) {
                        ...
                        for (n in c(1:n_samples_C)) {
                          ...
                          sub_tmp_group_C <- tmp_group_B[which(
    ...
    if (univlog && !is.null(columns) &&
        ...
        if (pval_type == "Corrected by B-H") {
            testuniv$p.value <- round(
                p.adjust(testuniv$p.value, "BH"),
                6
            )
        } else if (pval_type == "Corrected by Bonferroni") {
            testuniv$p.value <- round(
                p.adjust(testuniv$p.value, "bonferroni"),
                6
            )
        }
...
big_PCA <- function(FBM, rows, columns) {
    ...
    if (length(columns) >
        ...
        return(
            list(
                Scores = PCA_error_perhaps$Scores, Variance_explained = PCA_error_perhaps$Variance_explained
            )
        )
