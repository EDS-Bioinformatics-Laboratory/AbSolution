...
app_server <- function(input, output, session) {
    ...
    lst1 <- reactive(
        {
            ...
            if (is.null(input$raw_files)) {
                ...
                # path_list <- as.list(input$raw_files$datapath) tbl_list <-
                # lapply(input$raw_files$datapath, read.table, header=TRUE,
                # sep=';')
    ...
    sample_table_react <- reactive(
        {
            validate(
                need(
                  input$raw_sample_file != "", "You need to upload your file with the sample information"
              )
            )
    ...
    dirname_AIRR <- reactive(
        {
            parseDirPath(volumes, input$preinfolder_AIRR)
        }
    )
                ...
                if (!all(is.numeric(unlist(input$preinfolder_AIRR)))) {
                  sample_summary_status$Found_in_folder[which(
                    paste(sample_summary_status$Filename, ".tsv", sep = "") %in%
                      list.files(
                        path = parseDirPath(volumes(), input$preinfolder_AIRR),
                        full.names = F
                    )
                )] <- "Yes"
                }
            ...
            if (all(
                  ...
                  sample_summary_status$Rds_and_Bk_files_found_in_folder[intersect(
                    which(
                      paste(sample_summary_status$Filename, ".bk", sep = "") %in%
                        list.files(
                          path = parseDirPath(volumes(), input$bigmem_folder),
                          full.names = F
                      )
                  ),
                    which(
                ...
                write.table(
                  sample_info_react$table, file.path(
                    parseDirPath(volumes, input$base_folder),
                    "Sample_summary.txt"
                ),
        ...
        input$Preprocess_AIRR, {
            ...
            for (row_number_sample_table in c(1:nrow(inf_table))) {
                ...
                parse_AIRRSeq_file(
                  ...
                  sample = inf_table$Sample[row_number_sample_table], input_path = paste(
                    parseDirPath(volumes(), input$preinfolder_AIRR),
                    "/", sep = ""
                ),
    ...
    output$make_dummy <- renderUI(
        {
            if (is.null(input$raw_sample_file) ||
                all(is.numeric(unlist(input$bigmem_folder)))) {
            } else {
    ...
    observeEvent(
        input$Feature_determination, {
            ...
            Features_values$Total <- Feature_1(
                parseDirPath(volumes, input$base_folder),
            ...
            for (i in c(1:length(List_dfs))) {
                ...
                Feature__dataset(
                  path_base = parseDirPath(volumes, input$base_folder),
        ...
        input$Move_to_analysis_real, {
            ...
            updateSliderInput(
                inputId = "Rmut_filter", min = min(
                  Big_mem_values$Big_DF[, which(Big_mem_values$Header == "AA_Whole_Replacement_muts")]
              ),
                max = max(
                  Big_mem_values$Big_DF[, which(Big_mem_values$Header == "AA_Whole_Replacement_muts")]
              )
            ...
            suppressWarnings(
                {
                  ...
                  Big_mem_color_values$VJ <- Ab_palette(
                    list_values[c("V", "J")],
                  ...
                  if ("Best_D" %in% colnames(Big_mem_values$Short_DF)) {
                    ...
                    Big_mem_color_values$VDJ <- Ab_palette(
                      list_values[c("V", "D", "J")],
            ...
            # test$Chain[1] seecol(usecol(pal = Ab_palette(list_values,
            # vect_genes_comb=sort(unique(test$V_and_J)), type_values=c('VJ')),
            # n = 'all')) seecol(usecol(pal = Ab_palette(list_values[['V']],
            # vect_genes_comb=NA, type_values=c('V')), n = 'all'))
            # seecol(usecol(pal = Ab_palette(list_values[['D']],
            # vect_genes_comb=NA, type_values=c('D')), n = 'all'))
            # seecol(usecol(pal = Ab_palette(list_values[['J']],
            # vect_genes_comb=NA, type_values=c('J')), n = 'all'))
            # seecol(usecol(pal = Ab_palette(list_values[c('V','D','J')],
            # vect_genes_comb=sort(unique(test$V_and_D_and_J)),
            # type_values=c('VDJ')), n = 'all'))


            tmp_rows_cols <- filter_merged(
    ...
    observeEvent(
        toListenSelection(), {
            if (!is.null(Selection_values$rows) &&
                ...
                print(
                  paste(
                    "PCAAAA:", length(Selection_values$rows),
                    sep = ""
                )
              )
                tmp_PCA <- big_PCA(
                ...
                print(
                  paste(
                    "PCAAAA Scores:", nrow(Selection_values$Scores),
                    sep = ""
                )
              )
                ...
                if (input$use_UMAP == T && as.numeric(
                  unname(
                    strsplit(
                      as.character(print(get_ram(), unit_system = "metric")),
                      split = " "
                  )[[1]][1]
                )
              ) >
                  ...
                  tmp_umap <- as.data.frame(
                    umap(Big_mem_values$Big_DF[Selection_values$rows, Selection_values$columns])$layout
                )
                  print(dim(tmp_umap))
    ...
    observeEvent(
        toListenExploration(), {
            if (!is.null(Exploration_values$rows) &&
                ...
                # if (all(Exploration_values$rows==Selection_values$rows) &&
                # all(Exploration_values$columns==Selection_values$columns) ) {
                ...
                if (input$use_UMAP && as.numeric(
                  ...
                  print("UMAPEx")
                  tmp_umap <- as.data.frame(
                    umap(Big_mem_values$Big_DF[Exploration_values$rows, Exploration_values$columns])$layout
                )
                  print(dim(tmp_umap))
                  colnames(tmp_umap) <- paste(
                    "Dim_", c(1:2),
    ...
    output$Selection_plot <- renderPlotly(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                if (input$plot_color == "Best_V") {
                  ...
                  Sel_colors <- Ab_palette(
                    list_values = unique(Selection_values$Scores$Color),
                    vect_genes_comb = NA, type_values = "cualitative", colorblind = F
                )
                ...
                fig <- plot_ly(data = test, type = "scatter", mode = "markers", colors = Sel_colors) %>%
                  config(
                    toImageButtonOptions = list(
                      format = "png", filename = "PCA_Selection_plot", width = 1400,
                      height = 1000
                  )
                )
                ...
                if (nrow(tmp_test) >
                  ...
                  if (length(not_in_leg) >
                    ...
                    fig <- fig %>%
                      add_trace(
                        data = tmp_tmp_test, x = ~x, y = ~y, opacity = 0.9, text = ~Text_ID,
                        key = ~Text_ID, color = ~Color, symbol = ~Symbol, type = "scatter",
                        mode = "markers", fill = ~"", hovertemplate = paste("<b>%{text}</b>"),
                        marker = list(line = list(color = "black", width = tmp_tmp_test$Width_border[1])),
                        name = tmp_tmp_test$Color, showlegend = TRUE, legendroup = tmp_tmp_test$Color
                    )
                  }
                  tmp_tmp_test <- tmp_test[which(tmp_test$Color %!in% not_in_leg),
                    ]
                  if (nrow(tmp_tmp_test) >
                    ...
                    fig <- fig %>%
                      add_trace(
                        ...
                        mode = "markers", fill = ~"", hovertemplate = paste("<b>%{text}</b>"),
                        marker = list(line = list(color = "black", width = tmp_tmp_test$Width_border[1])),
                        name = tmp_tmp_test$Color, showlegend = FALSE, legendroup = tmp_tmp_test$Color
                    )
                  }
                ...
                fig <- fig %>%
                  layout(
                    title = "Selection plot", plot_bgcolor = "#e5ecf6", legend = list(orientation = "v", y = 0),
                  ...
                  config(
                    displaylogo = FALSE, modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "autoScale2d", "pan2d", "hoverCompareCartesian")
                )
    ...
    output$Exploration_plot <- renderPlotly(
        {
            if (!is.null(Exploration_values$rows) &&
                ...
                # print(input$plot_color_expl) Ex_colors=Big_mem_color_values$V
                # } else if(input$plot_color_expl =='J_gene') {
                # Ex_colors=Big_mem_color_values$J } else
                # if(input$plot_color_expl =='D_gene') {
                # Ex_colors=Big_mem_color_values$D }else
                # if(input$plot_color_expl =='V_and_J') {
                # Ex_colors=Big_mem_color_values$VJ }else
                # if(input$plot_color_expl =='V_and_D_and_J') {
                # Ex_colors=Big_mem_color_values$VDJ }
                ...
                fig <- plot_ly(data = test, type = "scatter", mode = "markers", colors = Ex_colors) %>%
                  config(
                    toImageButtonOptions = list(
                      format = "png", filename = "PCA_exploration_plot", width = 1400,
                      height = 1000
                  )
                )
                ...
                fig <- fig %>%
                  add_trace(
                    data = tmp_test_ns, x = ~Dim_1, y = ~Dim_2, opacity = 0.7, text = ~Text_ID,
                    key = ~Text_ID, color = ~Color, colors = Ex_colors, type = "scatter",
                    mode = "markers", fill = ~"", hovertemplate = paste("<b>%{text}</b>"),
                    marker = list(sizemode = "diameter"),
                    name = tmp_test_ns$Color, legendroup = tmp_test_ns$Color
                )
                ...
                if (nrow(tmp_test) >
                  ...
                  fig <- fig %>%
                    add_trace(
                      ...
                      type = "scatter", mode = "markers", fill = ~"", hovertemplate = paste("<b>%{text}</b>"),
                      marker = list(line = list(color = "#ff4d6d", width = tmp_test$Width_border[1])),
                      name = tmp_test$Color, showlegend = FALSE, legendroup = tmp_test$Color
                  )
                }
                ...
                fig <- fig %>%
                  layout(
                    title = "Exploration plot", plot_bgcolor = "#e5ecf6", legend = list(orientation = "v", y = -0),
                    showlegend = T, xaxis = list(
        ...
        toListenColorsSelection(), {
            if (!is.null(input$plot_color_expl) &&
                ...
                if (input$plot_color_expl %in% c("Best_V", "Best_J")) {
                  # print('Test_1') print(tmp_sp[1:10]) tmp_sp=sapply(tmp_sp,
                  # function(z) strsplit(z,split='*', fixed=T)[[1]][1])
                  # print('Test_2')
                  print(tmp_sp[1:10])
                } else if (input$plot_color_expl == "V_and_J") {
                  # tmp_sp=sapply(tmp_sp, function(z)
                  # paste(strsplit(z,split='*', fixed=T)[[1]][1],
                  # strsplit(strsplit(z,split='_', fixed=T)[[1]][2], split='*',
                  # fixed=T)[[1]][1], sep=' & '))
                }
                ...
                print("Text_ID")
                print(
                  Exploration_values$Scores$Text_ID[which(Exploration_values$Scores$Text_ID %in% ID_selected_values$subclones$key)]
              )
                print(
                  old_sp[which(Exploration_values$Scores$Text_ID %in% ID_selected_values$subclones$key)]
              )
                print(
                  tmp_sp[which(Exploration_values$Scores$Text_ID %in% ID_selected_values$subclones$key)]
              )
                    ...
                    clonepart_start <- (Big_mem_values$Short_DF)[intersect(
                      Selection_values$rows, which(
                        (Big_mem_values$Short_DF)[, get(
                          paste(
                            "Clone", input$clonal_level_group, input$clonal_region_group,
                            input$clonal_group, input$identity_clonal_group, "simil",
                            sep = "_"
                        )
                      )] %in%
                          ID_selected_values$clones$key
        ...
        ID_selected_values$subclones, {
            ...
            if (!is.null(ID_selected_values$subclones) &&
                ...
                Big_mem_values$Short_DF[unique(
                  c(
                    match(
                      ID_selected_values$subclones$key, paste(
                        unlist(Big_mem_values$Short_DF[, get("ID")]),
                        unlist(Big_mem_values$Short_DF[, get("Sequence_type")]),
                        sep = "_&_"
                    )
                  ),
                ...
                print("a ver..3.")
                print(which(Selection_values$Scores$Selected == "Clones"))
                Big_mem_values$Short_DF[Selection_values$rows[which(Selection_values$Scores$Selected == "Clones")],
                  ..keep_cols]
            }
    ...
    output$Ab_table <- DT::renderDataTable(
        {
            ...
            DT::datatable(
                rendered_table, extensions = "Buttons", options = list(dom = "Bfrtip", buttons = c("copy", "csv"))
            )
        }, options = list(
    ...
    observeEvent(
        ignoreInit = TRUE, list(input$samples_selected, input$Rmut_filter),
        {
    ...
    output$VJ_selection <- renderUI(
        {
            ...
            pickerInput(
                ...
                choices = choices, selected = tmp_selection, options = pickerOptions(
    ...
    observeEvent(
        ignoreInit = TRUE, Big_mem_values$Short_DF, {
    ...
    observeEvent(
        ignoreInit = TRUE, list(input$Sample_selection, input$use_what, input$use_productive_or_not),
        {
            ...
            updateSliderInput(
                inputId = "Rmut_filter", min = min(
                  Big_mem_values$Big_DF[rows, which(Big_mem_values$Header == "AA_Whole_Replacement_muts")]
              ),
                max = max(
                  Big_mem_values$Big_DF[rows, which(Big_mem_values$Header == "AA_Whole_Replacement_muts")]
              )
    ...
    observeEvent(
        ignoreInit = TRUE, list(
            input$samples_selected, input$use_what, input$groups_selected, input$VJ_deselected,
            input$use_sharedVDJ, input$VDJ_normalized_per_size, input$my_clone_def,
                              ...
                              sapply(
                                ...
                                function(y) min(
                                  min_total_sizes_per_sample_groupA[z], length(
                                    unique(
                                      Big_mem_values$Short_DF[[input$my_clone_def]][intersect(
                                        intersect(
    ...
    output$VDJ_subsetting <- renderUI(
        {
            ...
            # if(length(labels)==0) { labels='No shared VJ' } print(labels[1])
            # print(intersect(which(Big_mem_values$Short_DF[[input$groups_selected]]
            # %in% input$group_A), which(Big_mem_values$Short_DF$Patient_Sample
            # %in% (input$samples_selected))))
            # print(Big_mem_values$Short_DF$V_and_J[1:3]) print(rows[1:10])
            # print(Big_mem_values$Short_DF$V_and_J[intersect(intersect(which(Big_mem_values$Short_DF[[input$groups_selected]]
            # %in% input$group_A), which(Big_mem_values$Short_DF$Patient_Sample
            # %in% (input$samples_selected))), rows)])
            # print(Big_mem_values$Short_DF$V_and_J[intersect(intersect(which(Big_mem_values$Short_DF[[input$groups_selected]]
            # %in% input$group_B), which(Big_mem_values$Short_DF$Patient_Sample
            # %in% (input$samples_selected))), rows)]) print(labels)
            # bucket_list( header = 'Select shared V-J combinations between
            # groups', orientation='horizontal', group_name =
            # 'V_J_classification', add_rank_list( text = 'To include', labels
            # = NULL, input_id='VJ_included' ), add_rank_list( text = 'To
            # exclude', labels = labels, input_id='VJ_excluded' ) )

            pickerInput(
                inputId = "VJ_included", label = "To include only the following VJ combinations:",
                choices = labels, options = pickerOptions(
    ...
    output$individual_variables <- renderUI(
        {
            ...
            # selectizeInput('exclude_variables', 'Excluding individual
            # variables (start writing the category):', choices=choices,
            # multiple = TRUE, options = list( # onInitialize =
            # I(onInitialize), onDropdownOpen = I(onDropdownOpen) ) )
            ...
            infoBox(
                title = "Selection dataset", value = tags$p(
                  if (as.numeric(
                    ...
                    "Fits in RAM, more ML methods are available. If both datasets fit in RAM, UMAP and PCA biplot will be available"
                  } else {
                    "Does not fit in RAM"
                  }, style = "font-size: 50%;"
              ),
            ...
            infoBox(
                title = "Exploration dataset", value = tags$p(
                  if (as.numeric(
                    ...
                    "Fits in RAM. If both datasets fit in RAM, UMAP and PCA biplot will be available."
                  } else {
                    "Does not fit in RAM"
                  }, style = "font-size: 50%;"
              ),
    ...
    output$correlated_variables <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                # for (keyvar in names(pairs_correlated)) { for (keyvar2 in
                # pairs_correlated[[keyvar]]) { if (as.numeric(keyvar) %!in%
                # pairs_correlated[[as.character(keyvar2)]]) {
                # print('MMMMMMMMMMMMWIRD') print(keyvar)
                # print(pairs_correlated[[(keyvar)]]) print(keyvar2)
                # print(pairs_correlated[[as.character(keyvar2)]]) } } }
                print(
                  paste(
                    "Correlated elements: ", length(index_correlated),
                    sep = ""
                )
              )
                print(
                  paste(
                    "Correlated elements bis: ", length(unique(unlist(pairs_correlated))),
                    sep = ""
                )
              )
    ...
    output$numerical_covariables_plot <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                # gene_table2=readRDS('/home/rgarcia/Descargas/Dani_genes/expression_matrix.rds',
                # refhook = NULL) index_null=which(sapply(1:nrow(gene_table),
                # function(z) sum(gene_table[z, 2:ncol(gene_table)])) ==0)
                # gene_table=t(gene_table) colnames(gene_table)=gene_table[1,]
                # gene_table=gene_table[2:nrow(gene_table),]
                # gene_table$Cell_ID=rownames(gene_table)
                # write.table(gene_table,
                # file='/home/rgarcia/Descargas/Dani_genes/expression_matrix_cor.txt',
    ...
    output$Group_selection_for_feature <- renderUI(
        {
            if (is.null(Selection_values$rows) &&
                ...
                length(Selection_values$columns) >
                  0) {
                selectInput(
                  "plot_color_feature", "Color by", choices = c("Sample", "Patient", "Group", "Subgroup", "Chain"),
                  selected = "Group"
              )
            } else {
                sample_info <- sample_info_react$table
    ...
    output$Violin_feature_plot <- renderPlotly(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                fig_violin <- draw_feature_violinplot(
                  ...
                  show_reconstructed = input$show_reconstructed, selected_rows = Selection_values$rows,
                  selected_subclones = NULL, hide_dots = input$hide_points
              )
                ID_selected_values$subclones <- event_data("plotly_selected")
        ...
        input$calculate_new_clone, {
            if (!is.null(Selection_values$rows) &&
                ...
                length(Selection_values$columns) >
                  0) {
                if (!(paste(
                  "Clone", input$clonal_level_group, input$clonal_region_group, input$new_clonal_group,
                  input$identity_clonal_group, "simil", sep = "_"
              ) %in%
                  colnames(Big_mem_values$Short_DF))) {
                  print("2/2")
                  Big_mem_values$Short_DF <- calculate_clone(
                  ...
                  print("DONE")
                  print(
                    colnames(Big_mem_values$Short_DF)[(length(Big_mem_values$Short_DF) -
                      10):length(Big_mem_values$Short_DF)]
                )
                }
            }
        }
    ...
    output$Comparison_plot <- renderPlotly(
        {
            if (!is.null(ID_selected_values$intersection_samples$setNames) &&
                ...
                if (length(unlist(ID_selected_values$intersection_samples$setNames)) >
                  1) {
                  fig_sharedclones <- draw_sharedclonesplot(
    ...
    output$ML_clone_def_to_use <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                length(Selection_values$columns) >
                  0) {
                selectInput(
                  "MLclone", "Clone definitions yet to generate, go to 5.", choices = NULL,
                  selected = NULL
              )
            } else {
                selectInput(
                  "MLclone", "Clone definition", choices = colnames(Big_mem_values$Short_DF)[which(
                    startsWith(
                      colnames(Big_mem_values$Short_DF),
                      "Clone"
                  )
    ...
    output$ML_group_selection <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                length(Selection_values$columns) >
                  0) {
                selectInput(
                  "groups_selected", "Separate", choices = c("Group"),
                  selected = "Group"
              )
            } else {
                selectInput(
                  "groups_selected", "Separate", choices = c("Group", "Patients"),
                  selected = "Group"
              )
            }
    ...
    output$ML_method_selection <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                length(Selection_values$columns) >
                  0) {
                selectInput(
                  "groups_selected_ML", "Work only with", choices = c("Linear regression"),
                ...
                if ((as.numeric(
                  ...
                  selectInput(
                    "groups_selected_ML", "Work only with", choices = c("Random Forest", "Linear regression", "SVM"),
    ...
    output$ML_assign_sample_to_training <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                sample_info <- sample_info_react$table
                selectizeInput(
                  "patients_to_exclude", "Select the subjects to include only in the ML validation (if used, this overrides the % size of samples destined to training and validation sets as the other subjects will be used as training)",
                  choices = sample_info$Patient, selected = NULL, multiple = TRUE,
    ...
    ML_values <- reactiveValues(
        models_generated = list(), current_training_rows = NULL, current_validation_rows = NULL,
        group_category = NULL
    )

    observeEvent(
        input$update_ML_group_parameters, {
            ...
            if (input$ML_use_percentage_selection_dataset != 100) {
                ...
                for (subject in unique(Big_mem_values$Short_DF[tmp_rows, get("Patient")])) {
                  ...
                  tmp_intersection <- tmp_rows[which(
                    Big_mem_values$Short_DF[tmp_rows, get("Patient")] ==
                      subject
                )]

                  tmp_decimated_rows <- c(
                    tmp_decimated_rows, sample(
                      tmp_intersection, round(input$ML_use_percentage_selection_dataset * length(tmp_intersection)/100)
                  )
                )
                }
            ...
            if (!is.null(input$patients_to_exclude)) {
                ...
                for (subject in input$patients_to_exclude) {
                  validation_rows <- c(
                    validation_rows, tmp_rows[which(
                      Big_mem_values$Short_DF[tmp_rows, get("Patient")] ==
                        subject
                  )]
                )
            ...
            if (input$ML_equal_VJs && input$ML_equal_set_sizes) {
                ...
                for (group in unique(Big_mem_values$Short_DF[tmp_rows, ..groupselection])) {
                  ...
                  tmp_VJ <- unique(
                    paste(
                      Big_mem_values$Short_DF[tmp_group_rows, get("Best_V")],
                      Big_mem_values$Short_DF[tmp_group_rows, get("Best_J")],
                      sep = "_&_"
                  )
                )
                  if (length(VJs) ==
                ...
                for (group in unique(Big_mem_values$Short_DF[tmp_rows, ..groupselection])) {
                  ...
                  for (j in c(1:length(VJ_sizes))) {
                    tmp_new_tmp_rows <- c(
                      tmp_new_tmp_rows, sample(
                        tmp_group_rows[which(tmp_VJ %in% names(VJ_sizes)[j])],
                        VJ_sizes[j]
                    )
                  )
            ...
            size_validation_compensate <- length(tmp_rows) -
                size_training - length(validation_rows)
            ...
            training_rows <- sample(
                tmp_rows[tmp_rows %!in% validation_rows], round(
                  percentage_to_compensate * (length(tmp_rows[tmp_rows %!in% validation_rows]))/100
              )
            )
    ...
    output$ML_sunburst_dataset_ML <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                length(Selection_values$rows) >
                  0 && !is.null(Selection_values$columns) &&
                length(Selection_values$columns) >
                  0 && length(ML_values$current_validation_rows) >
                0 && length(ML_values$current_validation_rows) >
                0) {
                rows <- c(ML_values$current_training_rows, ML_values$current_validation_rows)

                tmp_ML <- c(
                  rep("Training", length(ML_values$current_training_rows)),
                  rep("Validation", length(ML_values$current_validation_rows))
              )
                ...
                df_sunburst <- data.frame(
                  V1 = names(tmp_sunburst),
                  V2 = unname(tmp_sunburst)
              )


                sund2b(
                  df_sunburst, showLabels = F, rootLabel = "Training and validation set composition"
              )
    ...
    output$ML_sunburst_dataset_grouping <- renderUI(
        {
            if (!is.null(Selection_values$rows) &&
                ...
                tmp_subjects <- Big_mem_values$Short_DF[rows, get("Patient")]
                tmp_VJ <- paste(
                  Big_mem_values$Short_DF[rows, get("Best_V")],
                  Big_mem_values$Short_DF[rows, get("Best_J")],
                  sep = "_&_"
              )
    ...
    output$HELP_output <- renderUI(
        {
            if (HELP_values$stage == 0) {
                HTML(
                  "In this tab (0.Project information) we will define what type of project, samples and workspace will be used. You need to indicate:<br>
           <ul>
           ...
           "
                ...
                HTML(
                  "In this tab (1.AIRR-Seq conversion) we will reconstruct the repertoire germlines based on your input. You need to indicate:<br>
           <ul>
    ...
    observeEvent(
        input$Move_to_analysis, {
            ...
            hideTab(inputId = "menutabset", target = "2.Sequence feature determination")
            showTab(
                inputId = "menutabset", target = "1&2. Select your bigmem files",
                select = T
            )
        }
    )
