...
- Code/doc/AbSolution.pdf; R Markdown/vignette for the AbSolution package

**How did you test the code:**
- Unit tests present under tests/testthat/. Run with devtools::test() and devtools::check().
- Manual exploration of results of test dataset.

**Script 0:** dev/*.R - Golem scripts to set-up the package and update information
**Script 1:** R/run_app.R - Entry point to launch the Shiny app (`run_app()`).
**Script 2:** R/app_ui.R - Defines the Shiny UI layout and pages/modules.
**Script 3:** R/app_server.R - Defines server logic and module wiring.
**Script 4:** R/mod_* files - module scripts used by UI and server.
**Script 5:** R/fct/util_* files - function and util scripts, organized by step in which they are used.
...
- `run_app()` calls `shiny::shinyApp(ui = app_ui(), server = app_server)` to launch the application.
- `app_ui()` composes the interface and modules; `app_server()` registers module servers and reactivity.
- Module scripts under `R/mod_*`, `R/fct_*`, `R/util_*` define UI and server helpers used by `app_ui/app_server`.
- Code/doc/AbSolution.pdf; explains how to use the application after `run_app()`
